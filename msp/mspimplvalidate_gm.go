...
func (msp *bccspgmmsp) validateIdentity(id *gmidentity) error {
	...
	if err != nil {
		return errors.WithMessage(err, "could not validate gmidentity against certification chain")
	}

	err = msp.internalValidateIdentityOusFunc(id)
	if err != nil {
		return errors.WithMessage(err, "could not validate gmidentity's OUs")
	}

	return nil
}

func (msp *bccspgmmsp) validateCAIdentity(id *gmidentity) error {
	if !id.cert.IsCA {
...
func (msp *bccspgmmsp) validateTLSCAIdentity(cert *x509.Certificate, opts *x509.VerifyOptions) error {
	if !cert.IsCA {
		return errors.New("Only CA identities can be validated")
	}
...
func (msp *bccspgmmsp) validateIdentityAgainstChain(id *gmidentity, validationChain []*x509.Certificate) error {
	return msp.validateCertAgainstChain(id.cert, validationChain)
}

func (msp *bccspgmmsp) validateCertAgainstChain(cert *x509.Certificate, validationChain []*x509.Certificate) error {
	// here we know that the gmidentity is valid; now we have to check whether it has been revoked

	// identify the SKI of the CA that signed this cert
	SKI, err := getSubjectKeyGMIdentifierFromCert(validationChain[1])
	if err != nil {
	...
	for _, crl := range msp.CRL {
		aki, err := getAuthorityKeyGMIdentifierFromCrl(crl)
		if err != nil {
...
func (msp *bccspgmmsp) validateIdentityOUsV1(id *gmidentity) error {
	// Check that the gmidentity's OUs are compatible with those recognized by this MSP,
	...
	if len(msp.ouIdentifiers) > 0 {
		...
		if !found {
			if len(id.GetOrganizationalUnits()) == 0 {
				return errors.New("the gmidentity certificate does not contain an Organizational Unit (OU)")
			}
			return errors.Errorf("none of the gmidentity's organizational units [%v] are in MSP %s", id.GetOrganizationalUnits(), msp.name)
		}
	}
...
func (msp *bccspgmmsp) validateIdentityOUsV11(id *gmidentity) error {
	...
	// Make sure that the gmidentity has only one of the special OUs
	...
	if counter != 1 {
		return errors.Errorf("the gmidentity must be a client, a peer or an orderer gmidentity to be valid, not a combination of them. OUs: [%v], MSP: [%s]", id.GetOrganizationalUnits(), msp.name)
	}

	return nil
}

func (msp *bccspgmmsp) getValidityOptsForCert(cert *x509.Certificate) x509.VerifyOptions {
...
type authorityKeyGMIdentifier struct {
	KeyIdentifier             []byte  `asn1:"optional,tag:0"`
	AuthorityCertIssuer       []byte  `asn1:"optional,tag:1"`
	AuthorityCertSerialNumber big.Int `asn1:"optional,tag:2"`
}

// getAuthorityKeyGMIdentifierFromCrl returns the Authority Key Identifier
...
func getAuthorityKeyGMIdentifierFromCrl(crl *pkix.CertificateList) ([]byte, error) {
	aki := authorityKeyGMIdentifier{}

	for _, ext := range crl.TBSCertList.Extensions {
		// Authority Key Identifier is identified by the following ASN.1 tag
		// authorityKeyGMIdentifier (2 5 29 35) (see https://tools.ietf.org/html/rfc3280.html)
	...
	return nil, errors.New("authorityKeyGMIdentifier not found in certificate")
}

// getSubjectKeyGMIdentifierFromCert returns the Subject Key Identifier for the supplied certificate
// Subject Key Identifier is an identifier of the public key of this certificate
func getSubjectKeyGMIdentifierFromCert(cert *x509.Certificate) ([]byte, error) {
	var SKI []byte
