...
// gmmspSetupFuncType is the prototype of the setup function
type gmmspSetupFuncType func(config *m.FabricMSPConfig) error

// gmvalidateIdentityOUsFuncType is the prototype of the function to validate gmidentity's OUs
type gmvalidateIdentityOUsFuncType func(id *gmidentity) error

// gmsatisfiesPrincipalInternalFuncType is the prototype of the function to check if principals are satisfied
type gmsatisfiesPrincipalInternalFuncType func(id Identity, principal *m.MSPPrincipal) error

// This is an instantiation of an MSP that
// uses BCCSP for its cryptographic primitives.
type bccspgmmsp struct {
	...
	internalSetupFunc gmmspSetupFuncType

	// internalValidateIdentityOusFunc is the pointer to the function to validate gmidentity's OUs
	internalValidateIdentityOusFunc gmvalidateIdentityOUsFuncType

	// internalSatisfiesPrincipalInternalFunc is the pointer to the function to check if principals are satisfied
	internalSatisfiesPrincipalInternalFunc gmsatisfiesPrincipalInternalFuncType
...
func newBccspGMMsp(version MSPVersion) (MSP, error) {
...
func (msp *bccspgmmsp) getCertFromPem(idBytes []byte) (*x509.Certificate, error) {
	if idBytes == nil {
...
func (msp *bccspgmmsp) getIdentityFromConf(idBytes []byte) (Identity, bccsp.Key, error) {
	// get a cert
...
func (msp *bccspgmmsp) getSigningIdentityFromConf(sidInfo *m.SigningIdentityInfo) (SigningIdentity, error) {
	if sidInfo == nil {
		return nil, errors.New("getIdentityFromBytes error: nil sidInfo")
	}

	// Extract the public part of the gmidentity
	...
	peerSigner :=gmsw.GetSM2PrivKey(privKey)
	
	if err != nil {
		return nil, errors.WithMessage(err, "getIdentityFromBytes error: Failed initializing bccspCryptoSigner")
	}

	return newSigningGMIdentity(idPub.(*gmidentity).cert, idPub.(*gmidentity).pk, peerSigner, msp)
}
...
func (msp *bccspgmmsp) Setup(conf1 *m.MSPConfig) error {
	if conf1 == nil {
...
func (msp *bccspgmmsp) GetVersion() MSPVersion {
	return msp.version
}

// GetType returns the type for this MSP
func (msp *bccspgmmsp) GetType() ProviderType {
	return FABRIC
}

// GetIdentifier returns the MSP identifier for this instance
func (msp *bccspgmmsp) GetIdentifier() (string, error) {
	return msp.name, nil
}

// GetTLSRootCerts returns the root certificates for this MSP
func (msp *bccspgmmsp) GetTLSRootCerts() [][]byte {
	return msp.tlsRootCerts
}

// GetTLSIntermediateCerts returns the intermediate root certificates for this MSP
func (msp *bccspgmmsp) GetTLSIntermediateCerts() [][]byte {
	return msp.tlsIntermediateCerts
}

// GetDefaultSigningIdentity returns the
// default signing gmidentity for this MSP (if any)
func (msp *bccspgmmsp) GetDefaultSigningIdentity() (SigningIdentity, error) {
	mspLogger.Debugf("Obtaining default signing gmidentity")

	if msp.signer == nil {
		return nil, errors.New("this MSP does not possess a valid default signing gmidentity")
	}
...
// gmidentity identified by the supplied identifier
func (msp *bccspgmmsp) GetSigningIdentity(identifier *IdentityIdentifier) (SigningIdentity, error) {
	// TODO
	return nil, errors.Errorf("no signing gmidentity for %#v", identifier)
}

// Validate attempts to determine whether
// the supplied gmidentity is valid according
// to this MSP's roots of trust; it returns
// nil in case the gmidentity is valid or an
// error otherwise
func (msp *bccspgmmsp) Validate(id Identity) error {
	mspLogger.Debugf("MSP %s validating gmidentity", msp.name)

	switch id := id.(type) {
	// If this gmidentity is of this specific type,
	// this is how I can validate it given the
	// root of trust this MSP has
	case *gmidentity:
		return msp.validateIdentity(id)
	default:
		return errors.New("gmidentity type not recognized")
	}
}

// hasOURole checks that the gmidentity belongs to the organizational unit
...
func (msp *bccspgmmsp) hasOURole(id Identity, mspRole m.MSPRole_MSPRoleType) error {
	// Check NodeOUs
	if !msp.ouEnforcement {
		return errors.New("NodeOUs not activated. Cannot tell apart identities.")
	}

	mspLogger.Debugf("MSP %s checking if the gmidentity is a client", msp.name)
...
func (msp *bccspgmmsp) hasOURoleInternal(id *gmidentity, mspRole m.MSPRole_MSPRoleType) error {
	...
	return fmt.Errorf("The gmidentity does not contain OU [%s], MSP: [%s]", mspRole, msp.name)
}

// DeserializeIdentity returns an Identity given the byte-level
// representation of a SerializedIdentity struct
func (msp *bccspgmmsp) DeserializeIdentity(serializedID []byte) (Identity, error) {
	mspLogger.Debug("Obtaining gmidentity")
...
// deserializeIdentityInternal returns an gmidentity given its byte-level representation
func (msp *bccspgmmsp) deserializeIdentityInternal(serializedIdentity []byte) (Identity, error) {
...
// SatisfiesPrincipal returns null if the gmidentity matches the principal or an error otherwise
func (msp *bccspgmmsp) SatisfiesPrincipal(id Identity, principal *m.MSPPrincipal) error {
	principals, err := collectgmPrincipals(principal, msp.GetVersion())
	if err != nil {
		return err
	}
...
// collectgmPrincipals collects principals from combined principals into a single MSPPrincipal slice.
func collectgmPrincipals(principal *m.MSPPrincipal, mspVersion MSPVersion) ([]*m.MSPPrincipal, error) {
	...
	case m.MSPPrincipal_COMBINED:
		...
		// Recursively call msp.collectgmPrincipals for all combined principals.
		...
		for _, cp := range principals.Principals {
			internalSlice, err := collectgmPrincipals(cp, mspVersion)
			if err != nil {
				return nil, err
			}
...
// satisfiesPrincipalInternalPreV13 takes as arguments the gmidentity and the principal.
// The function returns an error if one occurred.
// The function implements the behavior of an MSP up to and including v1.1.
func (msp *bccspgmmsp) satisfiesPrincipalInternalPreV13(id Identity, principal *m.MSPPrincipal) error {
	switch principal.PrincipalClassification {
	// in this case, we have to check whether the
	// gmidentity has a role in the msp - member or admin
	case m.MSPPrincipal_ROLE:
		...
		// identifier is the same as that of the gmidentity
		if mspRole.MspIdentifier != msp.name {
			return errors.Errorf("the gmidentity is a member of a different MSP (expected %s, got %s)", mspRole.MspIdentifier, id.GetMSPIdentifier())
		}
		...
		case m.MSPRole_MEMBER:
			...
			// whether this gmidentity is valid for the MSP
			mspLogger.Debugf("Checking if gmidentity satisfies MEMBER role for %s", msp.name)
			return msp.Validate(id)
		case m.MSPRole_ADMIN:
			mspLogger.Debugf("Checking if gmidentity satisfies ADMIN role for %s", msp.name)
			// in the case of admin, we check that the
			// id is exactly one of our admins
			for _, admincert := range msp.admins {
				if bytes.Equal(id.(*gmidentity).cert.Raw, admincert.(*gmidentity).cert.Raw) {
					// we do not need to check whether the admin is a valid gmidentity
					// according to this MSP, since we already check this at Setup time
					// if there is a match, we can just return
					return nil
				}
			}
			return errors.New("This gmidentity is not an admin")
		case m.MSPRole_CLIENT:
			fallthrough
		case m.MSPRole_PEER:
			mspLogger.Debugf("Checking if gmidentity satisfies role [%s] for %s", m.MSPRole_MSPRoleType_name[int32(mspRole.Role)], msp.name)
			if err := msp.Validate(id); err != nil {
				return errors.Wrapf(err, "The gmidentity is not valid under this MSP [%s]", msp.name)
			}

			if err := msp.hasOURole(id, mspRole.Role); err != nil {
				return errors.Wrapf(err, "The gmidentity is not a [%s] under this MSP [%s]", m.MSPRole_MSPRoleType_name[int32(mspRole.Role)], msp.name)
			}
			return nil
		default:
	...
	case m.MSPPrincipal_IDENTITY:
		// in this case we have to deserialize the principal's gmidentity
		// and compare it byte-by-byte with our cert
		principalId, err := msp.DeserializeIdentity(principal.Principal)
		if err != nil {
			return errors.WithMessage(err, "invalid gmidentity principal, not a certificate")
		}

		if bytes.Equal(id.(*gmidentity).cert.Raw, principalId.(*gmidentity).cert.Raw) {
	...
	case m.MSPPrincipal_ORGANIZATION_UNIT:
		...
		if OU.MspIdentifier != msp.name {
			return errors.Errorf("the gmidentity is a member of a different MSP (expected %s, got %s)", OU.MspIdentifier, id.GetMSPIdentifier())
		}

		// we then check if the gmidentity is valid with this MSP
		...
		// now we check whether any of this gmidentity's OUs match the requested one
		for _, ou := range id.GetOrganizationalUnits() {
...
// satisfiesPrincipalInternalV13 takes as arguments the gmidentity and the principal.
// The function returns an error if one occurred.
...
func (msp *bccspgmmsp) satisfiesPrincipalInternalV13(id Identity, principal *m.MSPPrincipal) error {
	switch principal.PrincipalClassification {
	case m.MSPPrincipal_COMBINED:
...
// getCertificationChain returns the certification chain of the passed gmidentity within this msp
func (msp *bccspgmmsp) getCertificationChain(id Identity) ([]*x509.Certificate, error) {
	...
	default:
		return nil, errors.New("gmidentity type not recognized")
	}
}

// getCertificationChainForBCCSPIdentity returns the certification chain of the passed bccsp gmidentity within this msp
func (msp *bccspgmmsp) getCertificationChainForBCCSPIdentity(id *gmidentity) ([]*x509.Certificate, error) {
	if id == nil {
		return nil, errors.New("Invalid bccsp gmidentity. Must be different from nil.")
	}
	...
	if id.cert.IsCA {
		return nil, errors.New("An X509 certificate with Basic Constraint: " +
			"Certificate Authority equals true cannot be used as an gmidentity")
	}

	return msp.getValidationChain(id.cert, false)
}

func (msp *bccspgmmsp) getUniqueValidationChain(cert *x509.Certificate, opts x509.VerifyOptions) ([]*x509.Certificate, error) {
	// ask golang to validate the cert for us based on the options that we've built at setup time
	if msp.opts == nil {
		return nil, errors.New("the supplied gmidentity has no verify options")
	}
	validationChains, err := cert.Verify(opts)
	if err != nil {
		return nil, errors.WithMessage(err, "the supplied gmidentity is not valid")
	}

	// we only support a single validation chain;
	// if there's more than one then there might
	// be unclarity about who owns the gmidentity
...
func (msp *bccspgmmsp) getValidationChain(cert *x509.Certificate, isIntermediateChain bool) ([]*x509.Certificate, error) {
...
// getCertificationChainIdentifier returns the certification chain identifier of the passed gmidentity within this msp.
// The identifier is computes as the SHA256 of the concatenation of the certificates in the chain.
func (msp *bccspgmmsp) getCertificationChainIdentifier(id Identity) ([]byte, error) {
	...
	// chain[0] is the certificate representing the gmidentity.
	// It will be discarded
	return msp.getCertificationChainIdentifierFromChain(chain[1:])
}

func (msp *bccspgmmsp) getCertificationChainIdentifierFromChain(chain []*x509.Certificate) ([]byte, error) {
	// Hash the chain
...
func (msp *bccspgmmsp) sanitizeCert(cert *x509.Certificate) (*x509.Certificate, error) {
...
// IsWellFormed checks if the given gmidentity can be deserialized into its provider-specific form.
...
func (msp *bccspgmmsp) IsWellFormed(gmidentity *m.SerializedIdentity) error {
	bl, _ := pem.Decode(gmidentity.IdBytes)
	if bl == nil {
