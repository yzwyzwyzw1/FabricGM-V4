...
func (msp *bccspgmmsp) getCertifiersIdentifier(certRaw []byte) ([]byte, error) {
	...
	for _, v := range msp.rootCerts {
		if v.(*gmidentity).cert.Equal(cert) {
			found = true
			root = true
			break
		}
...
func (msp *bccspgmmsp) setupCrypto(conf *m.FabricMSPConfig) error {
...
func (msp *bccspgmmsp) setupCAs(conf *m.FabricMSPConfig) error {
	...
	// Recall that when an gmidentity is created, its certificate gets sanitized
	...
	for _, id := range msp.rootCerts {
		msp.opts.Roots.AddCert(id.(*gmidentity).cert)
	}
	for _, id := range msp.intermediateCerts {
		msp.opts.Intermediates.AddCert(id.(*gmidentity).cert)
	}

	return nil
}

func (msp *bccspgmmsp) setupAdmins(conf *m.FabricMSPConfig) error {
...
func (msp *bccspgmmsp) setupCRLs(conf *m.FabricMSPConfig) error {
...
func (msp *bccspgmmsp) finalizeSetupCAs() error {
	...
	for _, id := range append(append([]Identity{}, msp.rootCerts...), msp.intermediateCerts...) {
		if !id.(*gmidentity).cert.IsCA {
			return errors.Errorf("CA Certificate did not have the CA attribute, (SN: %x)", id.(*gmidentity).cert.SerialNumber)
		}
		if _, err := getSubjectKeyGMIdentifierFromCert(id.(*gmidentity).cert); err != nil {
			return errors.WithMessage(err, fmt.Sprintf("CA Certificate problem with Subject Key Identifier extension, (SN: %x)", id.(*gmidentity).cert.SerialNumber))
		}

		if err := msp.validateCAIdentity(id.(*gmidentity)); err != nil {
			return errors.WithMessage(err, fmt.Sprintf("CA Certificate is not valid, (SN: %s)", id.(*gmidentity).cert.SerialNumber))
		}
	}
	...
	for _, id := range append([]Identity{}, msp.intermediateCerts...) {
		chain, err := msp.getUniqueValidationChain(id.(*gmidentity).cert, msp.getValidityOptsForCert(id.(*gmidentity).cert))
		if err != nil {
			return errors.WithMessage(err, fmt.Sprintf("failed getting validation chain, (SN: %s)", id.(*gmidentity).cert.SerialNumber))
		}

		// Recall chain[0] is id.(*gmidentity).id so it does not count as a parent
...
func (msp *bccspgmmsp) setupNodeOUs(config *m.FabricMSPConfig) error {
...
func (msp *bccspgmmsp) setupSigningIdentity(conf *m.FabricMSPConfig) error {
	if conf.SigningIdentity != nil {
		...
		if expirationTime.After(now) {
			mspLogger.Debug("Signing gmidentity expires at", expirationTime)
		} else if expirationTime.IsZero() {
			mspLogger.Debug("Signing gmidentity has no known expiration time")
		} else {
			return errors.Errorf("signing gmidentity expired %v ago", now.Sub(expirationTime))
		}
...
func (msp *bccspgmmsp) setupOUs(conf *m.FabricMSPConfig) error {
...
func (msp *bccspgmmsp) setupTLSCAs(conf *m.FabricMSPConfig) error {
	...
	for _, cert := range append(append([]*x509.Certificate{}, rootCerts...), intermediateCerts...) {
		...
		if _, err := getSubjectKeyGMIdentifierFromCert(cert); err != nil {
...
func (msp *bccspgmmsp) setupV1(conf1 *m.FabricMSPConfig) error {
...
func (msp *bccspgmmsp) preSetupV1(conf *m.FabricMSPConfig) error {
	// setup crypto config
...
func (msp *bccspgmmsp) postSetupV1(conf *m.FabricMSPConfig) error {
...
func (msp *bccspgmmsp) setupV11(conf *m.FabricMSPConfig) error {
...
func (msp *bccspgmmsp) postSetupV11(conf *m.FabricMSPConfig) error {
