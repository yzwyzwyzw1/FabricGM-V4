...
func NewSignature(cred *Credential, sk *FP256BN.BIG, Nym *FP256BN.ECP, RNym *FP256BN.BIG, ipk *IssuerPublicKey, Disclosure []byte, msg []byte, rhIndex int, cri *CredentialRevocationInformation, rng *amcl.RAND,hashType string) (*Signature, error) {
	// Validate inputs
	...
	//c := HashModOrder("sha256",proofData)
	c := HashModOrder(hashType,proofData)
	...
	ProofC := HashModOrder(hashType,proofData)
...
func (sig *Signature) Ver(Disclosure []byte, ipk *IssuerPublicKey, msg []byte, attributeValues []*FP256BN.BIG, rhIndex int, revPkIn interface{}, epoch int) error {

	if ipk == nil || revPkIn == nil {
	...
	default:
		return  errors.Errorf("cannot support the publicKey ")
