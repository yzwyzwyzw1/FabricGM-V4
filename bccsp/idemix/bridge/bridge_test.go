...
import (
	...
	"github.com/chinaso/fabricGM/bccsp/idemix/bridge"
	"github.com/chinaso/fabricGM/bccsp/idemix/handlers"
	"github.com/chinaso/fabricGM/bccsp/idemix/handlers/mock"
...
var _ = Describe("Idemix Bridge", func() {
	...
	Describe("revocation", func() {
		...
		Context("sign", func() {
			...
			//	raw, err := Revocation.Sign(key, nil, 0, 0)
				//fmt.Println("raw",raw)
				//fmt.Println("err",err)
				//raw, err := Revocation.Sign(nil, nil, 0, 0)
	...
	Describe("signature", func() {
		...
		Context("sign", func() {
			It("fail on nil user secret key", func() {
				signature, err := SignatureScheme.Sign(nil, nil, nil, nil, nil, nil, nil, 0, nil,hashType)
				Expect(err.Error()).To(BeEquivalentTo("invalid user secret key, expected *Big, got [<nil>]"))
				Expect(signature).To(BeNil())
			})

			It("fail on nil nym public key", func() {
				signature, err := SignatureScheme.Sign(nil, userSecretKey, nil, nil, nil, nil, nil, 0, nil,hashType)
				Expect(err.Error()).To(BeEquivalentTo("invalid nym public key, expected *Ecp, got [<nil>]"))
				Expect(signature).To(BeNil())
			})

			It("fail on nil nym secret key", func() {
				signature, err := SignatureScheme.Sign(nil, userSecretKey, nymPublicKey, nil, nil, nil, nil, 0, nil,hashType)
				Expect(err.Error()).To(BeEquivalentTo("invalid nym secret key, expected *Big, got [<nil>]"))
				Expect(signature).To(BeNil())
			})

			It("fail on nil issuer public key", func() {
				signature, err := SignatureScheme.Sign(nil, userSecretKey, nymPublicKey, nymSecretKey, nil, nil, nil, 0, nil,hashType)
				Expect(err.Error()).To(BeEquivalentTo("invalid issuer public key, expected *IssuerPublicKey, got [<nil>]"))
				Expect(signature).To(BeNil())
			})
		})
	...
	Describe("setting up the environment with one issuer and one user", func() {
		...
		BeforeEach(func() {
			...
			fmt.Println("aaa",IssuerKey)
			Expect(err).NotTo(HaveOccurred())
			IssuerPublicKey, err = IssuerKey.PublicKey()
			Expect(err).NotTo(HaveOccurred())
			fmt.Println(IssuerPublicKey)
			// User
			User = &bridge.User{NewRand: bridge.NewRandOrPanic}
			...
			Expect(err).NotTo(HaveOccurred())
			fmt.Println("cccrt",credRequest)
			fmt.Println(CredentialRequestVerifier)
			fmt.Println("IssuerNonce",IssuerNonce)
			// Credential
			...
			fmt.Println("cred",credential)
			fmt.Println(CredentialVerifier)
			Expect(err).NotTo(HaveOccurred())
			...
			fmt.Println(RevocationPublicKey)
