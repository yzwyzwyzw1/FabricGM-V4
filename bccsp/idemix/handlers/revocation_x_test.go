	...
	Describe("when creating a revocation key-pair", func() {
		...
		Context("and the underlying cryptographic algorithm succeed", func() {
			...
			BeforeEach(func() {
				...
				// --------------------------------------------------- //
				idemixRevocationKeyType := handlers.NewRevocationSecretKey(idemixRevocationKey, false)

				fakeRevocationSecretKey = handlers.IsrevocationSecretKey(idemixRevocationKeyType)
			...
			Context("and the secret key is exportable", func() {
				BeforeEach(func() {
					...
					fakeRevocationSecretKey = handlers.IsrevocationSecretKey(handlers.NewRevocationSecretKey(idemixRevocationKey, true))
				})
			...
			Context("and the secret key is not exportable", func() {
				BeforeEach(func() {
					...
					fakeRevocationSecretKey = handlers.IsrevocationSecretKey(handlers.NewRevocationSecretKey(idemixRevocationKey, false))
				})
...
var _ = Describe("CRI", func() {
	...
	Describe("when creating a CRI", func() {
		...
		fmt.Println("CriSigner",CriSigner)
		...
		Context("and the underlying cryptographic algorithm succeed", func() {
			...
			It("returns no error and a signature", func() {
				signature, err := CriSigner.Sign(
					handlers.IsrevocationSecretKey(handlers.NewRevocationSecretKey(nil, false)),
					nil,
		...
		Context("and the underlying cryptographic algorithm succeed", func() {
			...
			It("returns no error and valid signature", func() {
				valid, err := CriVerifier.Verify(
					handlers.IsrevocationPublicKey(handlers.NewRevocationPublicKey(nil)),
					    []byte("fake signature"),
					    nil,
					     &bccsp.IdemixCRISignerOpts{},
				)
