...
// ---------------------------------------- //
func computeSKI(hashType string,serialise func() ([]byte, error)) ([]byte, error) {
	raw, err := serialise()
	if err != nil {
		return nil, err
	}

	if strings.Compare(hashType,"sha256") == 0 {
		hash := sha256.New()
		hash.Write(raw)
		return hash.Sum(nil), nil

	}else if strings.Compare(hashType,"sm3") == 0 {
		hash := sm3.New()
		hash.Write(raw)
		return hash.Sum(nil), nil
	}else {
		return nil,errors.Errorf("The name of the hash algorithm is error ")
	}

}

func NewNymSecretKey(hashType string,sk Big, pk Ecp, exportable bool) (*nymSecretKey, error) {
	ski, err := computeSKI(hashType,sk.Bytes)
	if err != nil {
		return nil, err
	}
...
func (k *nymSecretKey) PublicKey() (bccsp.Key, error) {
	...
	hashType := "sha256"
	ski, err := computeSKI(hashType,k.pk.Bytes)
	if err != nil {
		return nil, err
	}
...
func (kd *NymKeyDerivation) KeyDeriv(k bccsp.Key, opts bccsp.KeyDerivOpts) (dk bccsp.Key, err error) {
	...
	hashType := "sha256"
	return NewNymSecretKey(hashType,RandNym, Nym, kd.Exportable)
}
