...
func NewCA(baseDir, org, name, country, province, locality, orgUnit, streetAddress, postalCode string) (*CA, error) {
	...
	if err == nil {
		...
		if err == nil {
			...
			//获取私钥
		//	privkey := gmsw.GetSM2PrivKey(priv)
		//	fmt.Println(",privkey",privkey)
			...
			//	fmt.Println(",privkey",privkey)
				...
				x509Cert, err := genCertificateSM2(baseDir, name, &template, &template, privkey)
				response = err
				if err == nil {
					ca = &CA{
						Name:               name,
...
func (ca *CA) SignCertificate(baseDir, name string, ous, sans []string, pub *sm2.PublicKey,
	...
	fmt.Println("1adsffa")
	fmt.Println("ca.SignKey",ca.SignKey)
	fmt.Println("ca.SignCert",ca.SignCert)

	cert, err := genCertificateSM2(baseDir, name, &template, ca.SignCert, ca.SignKey)
...
func genCertificateSM2(baseDir, name string, template, parent *x509.Certificate, privKey *sm2.PrivateKey) (*x509.Certificate, error) {

    fmt.Println("privKey",privKey)
	fmt.Println("template",template)
	fmt.Println("parent",parent)
	//create the x509 public cert
	certBytes, err := x509.CreateCertificate(rand.Reader, template, parent, &privKey.PublicKey,privKey)
	if err != nil {
		return nil, err
	}
...
func LoadCertificateSM2(certPath string) (*x509.Certificate, error) {
	...
	walkFunc := func(path string, info os.FileInfo, err error) error {
		if strings.HasSuffix(path, ".pem") {
			...
			cert, err = gmutil.DERToX509Certificate(block.Bytes)
		}
		return nil
	}
