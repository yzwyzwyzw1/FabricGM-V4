...
func TestCreateCertificateSM2withSM3(t *testing.T) {
	...
	fmt.Println("sm2Content.sigAlgo yyy",sm2Content.sigAlgo)
	//2.创建证书模板
...
var Sm2templateCert = Certificate{
	// SerialNumber is negative to ensure that negative
	// values are parsed. This is due to the prevalence of
	// buggy code that produces certificates with negative
	// serial numbers.
...
func TestCreateCertificateRequestToPem(t *testing.T) {
	...
	fmt.Println("privKey.X",privKey.X)
	fmt.Println("privKey.Y",privKey.Y)
	fmt.Println("privKey.D",privKey.D)
...
BBDUw9yJuxRNB5yXBHj53RCaSPBVhwjkjszZysiPSZRGdpvDXP4W+Q/8lcssDeQD
MCqtuJDNTwM/IU+Lk5zg6hijggKFMIICgTAOBgNVHQ8BAf8EBAMCAgQwJgYDVR0l
...
HM9VAYN1A0gAMEUCIQCJV9pEFf1zNbaGramhyau/VaIKJPud+l+/sVWPh9XTMwIg
aW2NjrKSBDdQ602F9fDevcpBmXJaxkaHzWV3lTw4cYs=
-----END CERTIFICATE-----`
...
func TestCreateCertificateToPem(t *testing.T) {
	...
	template:=Sm2templateCert
	// ------------------------------------- //
	ok, err := sm2.WritePublicKeytoPem("pub.pem", &privKey.PublicKey, nil) // 生成公钥文件
	if ok != true {
		log.Fatal(err)
	}
	...
	ok, err = sm2.WritePrivateKeytoPem("priv.pem", privKey, nil) // 生成密钥文件
	if ok != true {
		log.Fatal(err)
	}
	...
	// --------------------------------------------------- //
	ok, _ = CreateCertificateToPem("cert.pem", &template, &template,  privKey)
	if ok != true {
	...
	//ans,err :=CreateCertificateToMem(&template, &template,  &privKey.PublicKey,privKey)
 //   fmt.Println("ans",ans)
//	fmt.Println("hex.EncodeToString(ans)",hex.EncodeToString(ans))
