...
import (
	...
	"github.com/chinaso/fabricGM/cryptopkg/golangGM/cryptobyte"
	cryptobyte_asn1 "github.com/chinaso/fabricGM/cryptopkg/golangGM/cryptobyte/asn1"
	"io"
	"math/big"
	"net"
	"net/url"
...
func (algo SignatureAlgorithm) String() string {
	...
	for _, details := range gmSignatureAlgorithmDetails {
		if details.algo == algo {
			return details.name
		}
	}
...
var gmSignatureAlgorithmDetails = []struct {
	algo       SignatureAlgorithm
	name       string
	oid        asn1.ObjectIdentifier
	pubKeyAlgo PublicKeyAlgorithm
...
func (c *Certificate) CheckSignature(algo SignatureAlgorithm, signed, signature []byte) error {
	...
	case *sm2.PublicKey:
		return checkSignatureSM(algo, signed, signature, c.PublicKey)
	default:
...
// CheckSignature verifies that signature is a valid signature over signed from
// a crypto.PublicKey.
func checkSignatureSM(algo SignatureAlgorithm, signed, signature []byte, pubKey interface{}) (err error) {
	//var hashType crypto.Hash
	publicKey:=pubKey.(*sm2.PublicKey)
	...
	if pubKeyAlgo == ECDSA {  //使用了ECDSA的公钥身份
...
// CheckSignature verifies that signature is a valid signature over signed from
// a crypto.PublicKey.
func checkSignature(algo SignatureAlgorithm, signed, signature []byte, publicKey PublicKey) (err error) {
	var hashType crypto.Hash
	var pubKeyAlgo PublicKeyAlgorithm
	...
	//var digest  []byte
	//if !hashType.Available() && !gmHashType.Available(){
	//	return ErrUnsupportedAlgorithm
	//}else{
	//
	//	if hashType.Available(){
...
func parsePublicKey(algo PublicKeyAlgorithm, keyData *publicKeyInfo) (interface{}, error) {
    ...
    // ----------------------------------------------- //
		if namedCurve.Params().Name == "SM2-P256" {
			pub :=&sm2.PublicKey{
				X:     x,
				Y:     y,
				Curve:namedCurve,
...
func parseCertificate(in *certificate) (*Certificate, error) {
	...
	fmt.Println("out.PublicKeyAlgorithm",out.PublicKeyAlgorithm)
...
func gmSigningParamsForPublicKey(pub interface{}, requestedSigAlgo SignatureAlgorithm) (hashFunc Hash, sigAlgo pkix.AlgorithmIdentifier, err error) {
	var pubType PublicKeyAlgorithm


	switch pub := pub.(type) {
	case *sm2.PublicKey:
		pubType = ECDSA   //我在这里默认使用了ECDSA的公钥身份,所以是这么书写的
		switch pub.Curve {
		case sm2.SM2P256():
	...
	fmt.Println("requestedSigAlgo",requestedSigAlgo)
	found := false
	for _, details := range gmSignatureAlgorithmDetails {
	...
	fmt.Println("sigAlgo.Algorithm",sigAlgo.Algorithm)
	fmt.Println("err",err)
...
//func signingParamsForPublicKey(pub interface{}, requestedSigAlgo SignatureAlgorithm) (hashFunc interface{}, sigAlgo pkix.AlgorithmIdentifier, err error) {
...
func CreateCertificate(rand io.Reader, template, parent *Certificate, pub, priv interface{}) (cert []byte, err error) {
	...
	case *ecdsa.PrivateKey,*rsa.PrivateKey:
		hashFunc, signatureAlgorithm, err1 := signingParamsForPublicKey(key.Public(), template.SignatureAlgorithm)
		if err1 == nil  {
	...
	case *sm2.PrivateKey:
		...
		gmHashFunc, gmSignatureAlgorithm, err2 := gmSigningParamsForPublicKey(key.Public(), template.SignatureAlgorithm)
		if err2 == nil{
			...
			c := tbsCertificate{
				Version:            2,
				SerialNumber:       template.SerialNumber,
				SignatureAlgorithm: gmSignatureAlgorithm,
				Issuer:             asn1.RawValue{FullBytes: asn1Issuer},
				Validity:           validity{template.NotBefore.UTC(), template.NotAfter.UTC()},
				Subject:            asn1.RawValue{FullBytes: asn1Subject},
				PublicKey:          publicKeyInfo{nil, publicKeyAlgorithm, encodedPublicKey},
			...
			var signerOpts crypto.SignerOpts
			signerOpts = crypto.Hash(gmHashFunc)
...
func (c *Certificate) CreateCRL(rand io.Reader, priv interface{}, revokedCerts []pkix.RevokedCertificate, now, expiry time.Time) (crlBytes []byte, err error) {
	...
	hashFunc, signatureAlgorithm, err1 := signingParamsForPublicKey(key.Public(), 0)
	if err1 == nil {
	...
	gmHashFunc, gmSignatureAlgorithm, err2 := gmSigningParamsForPublicKey(key.Public(), 0)
	if err2 == nil {
		...
		tbsCertList := pkix.TBSCertificateList{
			Version:             1,
			Signature:           gmSignatureAlgorithm,
			Issuer:              c.Subject.ToRDNSequence(),
			ThisUpdate:          now.UTC(),
			NextUpdate:          expiry.UTC(),
			RevokedCertificates: revokedCertsUTC,
...
//	var hashFuncX interface{}

	//var sigAlgo pkix.AlgorithmIdentifier


	//switch :hashFuncX.(type) {
	//case crypto.Hash:
	//	hashFunc := hashFuncX.(crypto.Hash)
	//case Hash:
	//	hashFunc :=hashFuncX.(Hash)
	//}
	...
	case *sm2.PublicKey:
		...
		fmt.Println("template.SignatureAlgorithm",template.SignatureAlgorithm)
		gmHashFunc, sigAlgo, err2 := gmSigningParamsForPublicKey(key.Public(), template.SignatureAlgorithm)
		if err2 == nil {
			...
			signature, err = key.Sign(rand, digest, crypto.Hash(gmHashFunc))
			//signature, err =
		...
		return nil, err
	default:
		hashFunc, sigAlgo, err1 := signingParamsForPublicKey(key.Public(), template.SignatureAlgorithm)
		if err1 == nil {
			h := hashFunc.New()
			h.Write(tbsCSRContents)
			digest := h.Sum(nil)
...
func (c *CertificateRequest) CheckSignature() error {
	...
	case *sm2.PublicKey:
		fmt.Println("sm2P")
		return checkSignatureSM(c.SignatureAlgorithm, c.RawTBSCertificateRequest, c.Signature, c.PublicKey)
	//case *ecdsa.PublicKey:
	//	fmt.Println("ecdsaP")
	default:
