...
func TestCloseServerConnectionOnIdleClient(t *testing.T) {
	clientConn, serverConn := net.Pipe()  //新建客户端,服务端连接通道
	server := Server(serverConn, testConfig.Clone()) //新建服务端TLS连接对象
	go func() {
		clientConn.Write([]byte{'0'})  //客户端写数据
		server.Close()  // 关闭TLS Server端
	}()
	server.SetReadDeadline(time.Now().Add(time.Second)) //设置读数据的时间期限
	err := server.Handshake()   //服务端握手
	if err != nil {
