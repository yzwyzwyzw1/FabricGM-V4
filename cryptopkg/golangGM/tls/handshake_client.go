...
NextCipherSuite:
	for _, suiteId := range possibleCipherSuites {
		for _, suite := range cipherSuites {
			...
			fmt.Println(suite)
			if suite.id==TLS_ECDHE_SM2_WITH_SM4_128_CBC_SM3 {
				fmt.Println("yzwyzwyzw",suite)
			}
...
func (c *Conn) clientHandshake() error {
	...
	hello, err := makeClientHello(c.config)  //执行发送clientHello动作
	if err != nil {
		return err
	}

	if c.handshakes > 0 {
		hello.secureRenegotiation = c.clientFinished[:] //安全重新协商
	}
	...
	if sessionCache != nil && c.handshakes == 0 {
		...
		// available.
		cacheKey = clientSessionCacheKey(c.conn.RemoteAddr(), c.config)  //获取CacheKey
		candidateSession, ok := sessionCache.Get(cacheKey) //获取候选会话ID
		if ok {
			// Check that the ciphersuite/version used for the
			// previous session are still valid.
			cipherSuiteOk := false
			for _, id := range hello.cipherSuites { //从hello消息中 获取密码套件
	...
	hs := &clientHandshakeState{//获取客户端握手状态
		c:       c,
		hello:   hello,
		session: session,
	}
...
// ----------------------------------- //
//handshake
//1.writeRecord 写hello消息的序列化数据到记录层
//2.readHandshake 读取Handshake信息
//3.解析服务端Hello消息
//4.pickTLSVersion 从服务端Hello消息中解析出TLS版本号
//5.pickCipherSuite 从服务端Hello消息之后解析出密码套件,即标准书中所说的密码学规范
//6.processServerHello 处理服务端Hello消息
//7.newFinishedHash 新建完成hash消息
//8.1 如果服务端的hello消息中提示要求重新发送消息
//8.1.1establishKeys 创建密钥
//8.1.2readSessionTicket 读取会话Ticket
//8.1.3readFinished 读取服务端Finshed消息
//8.1.4如果c.clientFinishedIsFirst = false,则sendFinished 读取服务端HelloFinshed消息
//8.2 如果服务端的hello消息中提示不是重发的消息,那么需要执行完整的握手协议
//8.2.1doFullHandshake 执行完整的握手协议
//8.2.2establishKeys 创建密钥
//8.2.3sendFinished 发送客户端Finshed消息
//8.2.4如果c.clientFinishedIsFirst = true,则读取会话Ticket消息
//8.2.5readFinished 读取服务端完成消息
...
func (hs *clientHandshakeState) handshake() error {
	...
	msg, err := c.readHandshake()  //客户端读握手信息
	if err != nil {
		return err
	}
	...
	if err = hs.pickTLSVersion(); err != nil {//发送TLS版本
		return err
	}
	...
	isResume, err := hs.processServerHello() //处理服务端Hello消息
	if err != nil {
		return err
	}

	hs.finishedHash = newFinishedHash(c.vers, hs.suite)   //做完成hash消息
	...
	if isResume {
		...
		if err := hs.doFullHandshake(); err != nil {  //执行握手消息
			return err
		}
		if err := hs.establishKeys(); err != nil { //创建密钥
			return err
		}
		if err := hs.sendFinished(c.clientFinished[:]); err != nil { //发送完成Finished消息
			return err
		}
		if _, err := c.flush(); err != nil {
			return err
		...
		if err := hs.readSessionTicket(); err != nil {  //读SessionTicket
			return err
		}
		if err := hs.readFinished(c.serverFinished[:]); err != nil { //读完成消息
			return err
		}
	}

	// ekmFromMasterSecret generates exported keying material
	c.ekm = ekmFromMasterSecret(c.vers, hs.suite, hs.masterSecret, hs.hello.random, hs.serverHello.random)
	c.didResume = isResume
	atomic.StoreUint32(&c.handshakeStatus, 1)  //标记握手状态信息为1
...
func (hs *clientHandshakeState) pickCipherSuite() error {
	...
	fmt.Println("pickCipherSuite  hs.suite", hs.suite)
...
// --------------------------------------------------- //
//执行完整的握手过程
//1.从客户端连接对象中读取连接消息,主要是根据握手的类型,新建握手数据结构
//2.如果是第一次握手的话就提取证书消息数据,再解析证书消息
//3.执行证书的验证过程
//4.readHandshake 从握手消息读取消息
//5.从消息中解析出证书状态信息
//6.获取处理器对象,调用服务器密钥扩展消息
//7.获取服务端发来的证书请求消息
//8.获取服务端发来的serverHelloDone消息
func (hs *clientHandshakeState) doFullHandshake() error {
	...
	keyAgreement := hs.suite.ka(c.vers)   //获取处理器函数
...
func (hs *clientHandshakeState) getCertificate(certReq *certificateRequestMsg) (*Certificate, error) {
	...
	if c.config.GetClientCertificate != nil {
		...
		if !certReq.hasSignatureAndHash {
			...
			// ----------------------------------------- //
			if !sm2Avail {
				fmt.Println("no implement,no support tls11")
			}
