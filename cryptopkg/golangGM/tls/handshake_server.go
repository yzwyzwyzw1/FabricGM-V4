...
func (c *Conn) serverHandshake() error {
	...
	c.config.serverInitOnce.Do(func() { c.config.serverInit(nil) }) //TLS 服务端初始化

	hs := serverHandshakeState{  //新建服务端握手结构体
		c: c,
	}
	isResume, err := hs.readClientHello()  //读取客户端hello信息
	if err != nil {
		return err
	}
	...
	if isResume {
		...
		if hs.hello.ticketSupported {
			if err := hs.sendSessionTicket(); err != nil {  //发送session消息
				return err
			}
		}
		if err := hs.sendFinished(c.serverFinished[:]); err != nil { //发送Finished消息
			return err
		}
...
func (hs *serverHandshakeState) doResumeHandshake() error { //重新开始握手
	c := hs.c
	...
	hs.finishedHash.discardHandshakeBuffer()   //关闭握手
	...
	if len(hs.sessionState.certificates) > 0 {
		if _, err := hs.processCertsFromClient(hs.sessionState.certificates); err != nil {  //处理客户端证书
			return err
		}
	}
...
func (hs *serverHandshakeState) sendFinished(out []byte) error {
	...
	if _, err := c.writeRecord(recordTypeChangeCipherSpec, []byte{1}); err != nil {    //recordTypeChangeCipherSpec
		return err
	}

	finished := new(finishedMsg)  //新finishedMsg对象
	finished.verifyData = hs.finishedHash.serverSum(hs.masterSecret)    //计算密钥和数据
	hs.finishedHash.Write(finished.marshal())  //完成hash写数据
	if _, err := c.writeRecord(recordTypeHandshake, finished.marshal()); err != nil { //写记录层消息
		return err
	}
...
func (hs *serverHandshakeState) processCertsFromClient(certificates [][]byte) (crypto.PublicKey, error) {
	...
	for i, asn1Data := range certificates {
		if certs[i], err = x509.ParseCertificate(asn1Data); err != nil {  //解析客户客户端证书
			c.sendAlert(alertBadCertificate)
	...
	if c.config.VerifyPeerCertificate != nil {
		if err := c.config.VerifyPeerCertificate(certificates, c.verifiedChains); err != nil {  //验证peer节点证书
			c.sendAlert(alertBadCertificate)
			return nil, err
		}
	}
