...
func (test *clientTest) connFromCommand() (conn *recordingConn, child *exec.Cmd, stdin opensslInput, stdout *opensslOutputSink, err error) {
	cert := testRSACertificate//测试RSA证书
	if len(test.cert) > 0 {
		cert = test.cert
	}
	...
	var key interface{} = testRSAPrivateKey   //定义一个RSA私钥接口
	if test.key != nil {
		key = test.key   //当不为空的时候赋值为test的私钥
	}
	var pemType string
	var derBytes []byte
	switch key := key.(type) {
	case *rsa.PrivateKey:
		pemType = "RSA"
		derBytes = x509.MarshalPKCS1PrivateKey(key)//序列化私钥
	...
	case *sm2.PrivateKey:
		pemType = "EC"
		var err error
		derBytes, err = sm2.MarshalSM2PrivateKey(key,nil)
		if err != nil {
			panic(err)
		}
	...
	keyPath := tempFile(string(pemOut.Bytes())) //创建一个包含私钥pem的临时文件,并且返回该文件的路径
	defer os.Remove(keyPath)                    //在结束时关闭
	...
	for i := uint(0); i < 5; i++ {
		tcpConn, err = net.DialTCP("tcp", nil, &net.TCPAddr{  //TCP连接通信
	...
	record := &recordingConn{   //构建记录层连接实例
		Conn: tcpConn,
	}
...
func (test *clientTest) run(t *testing.T, write bool) {
	checkOpenSSLVersion(t)  //检查openssl版本号
	...
	client := Client(clientConn, config) //实例化客户端对象,返回一个客户端结构体对象,并获得该对象的方法
	...
	if !write {
		flows, err := test.loadData()  //测试数据加载
		if err != nil {
		...
		for i, b := range flows {
			if i%2 == 1 {
				serverConn.Write(b)    //服务端连接写数据
				continue
			}
			fmt.Println("b",b)
			bb := make([]byte, len(b))
			_, err := io.ReadFull(serverConn, bb)//读文件
			fmt.Println("bb",bb)
			if err != nil {
	...
	if write {
		...
		recordingConn.WriteTo(out)  //记录层写数据
		fmt.Printf("Wrote %s\n", path)
	}
}
...
func runClientTestForVersion(t *testing.T, template *clientTest, prefix, option string) {
	...
	//解析Clienttest实例,以指针的方式传递给test
	test := *template
	test.name = prefix + test.name//字符拼接为新的test名
	if len(test.command) == 0 {
		test.command = defaultClientCommand//如果没有设定参数,就使用默认的参数,默认参数为 []string{"openssl", "s_client", "-no_ticket"}
	}

	test.command = append([]string(nil), test.command...)
	test.command = append(test.command, option)
	test.run(t, *update) //update =flag.Bool("update", false, "update golden files on disk")
}
...
func TestHandshakeClientRSAAES128GCM(t *testing.T) {
	...
	//将test传递给runClientTestTLS12,作为templatex参数传入
	runClientTestTLS12(t, test)
}
