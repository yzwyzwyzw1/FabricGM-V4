...
func (c *Conn) readHandshake() (interface{}, error) {
	for c.hand.Len() < 4 {
		...
		if err := c.readRecord(recordTypeHandshake); err != nil {  //循环检测握手过程是否完成
			return nil, err
		}
	}
...
func (c *Conn) Handshake() error {
	c.handshakeMutex.Lock()  //加锁
	defer c.handshakeMutex.Unlock()

	if err := c.handshakeErr; err != nil {
		return err
	}
	if c.handshakeComplete() {  //如果握手完成则返回
		return nil
	}
	...
	if c.isClient {
		fmt.Println("cccccc")
		c.handshakeErr = c.clientHandshake()  //客户端握手
	} else {
		c.handshakeErr = c.serverHandshake()  //服务端握手
	}
	...
	if c.handshakeErr == nil && !c.handshakeComplete() { //判断握手是否成功,达到最终的结果
