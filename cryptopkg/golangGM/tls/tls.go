...
func X509KeyPair(certPEMBlock, keyPEMBlock []byte) (Certificate, error) {
	...
	//	fmt.Println("certDERBlock",certDERBlock)
		if certDERBlock == nil {
			break
		}
	...
	//fmt.Println("len(skippedBlockTypes)",len(skippedBlockTypes))
	//fmt.Println("len(cert.Certificate) ",len(cert.Certificate) )
...
//	fmt.Println("111",skippedBlockTypes)
//fmt.Println("yyy")
	...
	//在执行x509.ParsePKCS8PrivateKey时根据公钥身份privKey.Algo.Algorith判断时,由于SM2使用的和ECDSA一样的公钥身份,所以需要先做公钥属性判断
	...
	case *sm2.PublicKey:
		...
		cert.PrivateKey, err = parsePrivateKeyGM(keyDERBlock.Bytes)
		if err != nil {
			return fail(err)
		}
...
func parsePrivateKeyGM(der []byte) (crypto.PrivateKey, error) {


	if key, err := sm2.ParsePKCS8UnecryptedSM2PrivateKey(der); err == nil {
