...
import (
	...
	"github.com/chinaso/fabricGM/cryptopkg/golangGM/grpc/credentials/alts/internal/testutil"
...
func TestClientHandshake(t *testing.T) {
	for _, testCase := range []struct {
		...
		for i := 0; i < testCase.numberOfHandshakes; i++ {
			...
			go func() {
				_, context, err := ClientHandshake(context.Background())
...
func TestServerHandshake(t *testing.T) {
	for _, testCase := range []struct {
		...
		for i := 0; i < testCase.numberOfHandshakes; i++ {
			...
			go func() {
				_, context, err := ServerHandshake(context.Background())
				if err == nil && context == nil {
					panic("expected non-nil ALTS context")
				}
				errc <- err
