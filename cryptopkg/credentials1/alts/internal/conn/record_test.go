...
func testPingPong(t *testing.T, np string) {
	...
	if n, err := Write(clientMsg); n != len(clientMsg) || err != nil {
		t.Fatalf("Client Write() = %v, %v; want %v, <nil>", n, err, len(clientMsg))
	}
	rcvClientMsg := make([]byte, len(clientMsg))
	if n, err := Read(rcvClientMsg); n != len(rcvClientMsg) || err != nil {
	...
	if n, err := Write(serverMsg); n != len(serverMsg) || err != nil {
		t.Fatalf("Server Write() = %v, %v; want %v, <nil>", n, err, len(serverMsg))
	}
	rcvServerMsg := make([]byte, len(serverMsg))
	if n, err := Read(rcvServerMsg); n != len(rcvServerMsg) || err != nil {
...
func testSmallReadBuffer(t *testing.T, np string) {
	...
	rcvMsg1 := rcvMsg[:n]
	rcvMsg2 := rcvMsg[n:]
	if n, err := Read(rcvMsg1); n != len(rcvMsg1) || err != nil {
		t.Fatalf("Read() = %v, %v; want %v, <nil>", n, err, len(rcvMsg1))
	}
	if n, err := Read(rcvMsg2); n != len(rcvMsg2) || err != nil {
...
func testLargeMsg(t *testing.T, np string) {
	...
	msgLen := altsRecordDefaultLength - msgTypeFieldSize - EncryptionOverhead() + 1
	msg := make([]byte, msgLen)
	if n, err := Write(msg); n != len(msg) || err != nil {
		t.Fatalf("Write() = %v, %v; want %v, <nil>", n, err, len(msg))
	}
	rcvMsg := make([]byte, len(msg))
...
func testFrameTooLarge(t *testing.T, np string) {
	...
	payloadLen := altsRecordLengthLimit - msgTypeFieldSize - EncryptionOverhead() + 1
	payload := make([]byte, payloadLen)
	c, err := Encrypt(nil, payload)
	if err != nil {
...
func testWriteLargeData(t *testing.T, np string) {
	...
	numberOfExpectedFrames := int(math.Ceil(float64(msgSize) / float64(payloadLengthLimit)))
	for i := 0; i < numberOfExpectedFrames; i++ {
		expectedRcvSize := payloadLengthLimit
		if i == numberOfExpectedFrames-1 {
			// Last frame might be smaller.
			expectedRcvSize = msgSize % payloadLengthLimit
		}
		tmpBuf := make([]byte, expectedRcvSize)
		if n, err := Read(tmpBuf); n != len(tmpBuf) || err != nil {
