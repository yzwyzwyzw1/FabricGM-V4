...
func TestCloneClient(t *testing.T) {
	...
	TargetServiceAccounts = []string{"not", "empty"}
	...
	if side != side {
		t.Errorf("cc.side = %q, want %q", side, side)
	}
	if hsAddress != hsAddress {
		t.Errorf("cc.hsAddress = %q, want %q", hsAddress, hsAddress)
	}
	if !reflect.DeepEqual(accounts, accounts) {
		t.Errorf("cc.accounts = %q, want %q", accounts, accounts)
	}
}
