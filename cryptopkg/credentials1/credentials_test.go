...
func TestTLSOverrideServerName(t *testing.T) {
	...
	c := NewTLS(nil)
	OverrideServerName(expectedServerName)
	if ServerName != expectedServerName {
		t.Fatalf("c.Info().ServerName = %v, want %v", ServerName, expectedServerName)
	}
}
...
func TestTLSClone(t *testing.T) {
	...
	cc := Clone()
	if ServerName != expectedServerName {
		t.Fatalf("cc.Info().ServerName = %v, want %v", ServerName, expectedServerName)
	}
	OverrideServerName("")
	if ServerName != expectedServerName {
		t.Fatalf("Change in clone should not affect the original, c.Info().ServerName = %v, want %v", ServerName, expectedServerName)
	}
...
func gRPCServerHandshake(conn net.Conn) (AuthInfo, error) {
	...
	_, serverAuthInfo, err := ServerHandshake(conn)
	if err != nil {
		return nil, err
	}
...
func gRPCClientHandshake(conn net.Conn, lisAddr string) (AuthInfo, error) {
	...
	_, authInfo, err := ClientHandshake(context.Background(), lisAddr, conn)
	if err != nil {
		return nil, err
	}
	return authInfo, nil
